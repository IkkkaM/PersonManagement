...
public interface IBaseRepository<T> where T : class
{
    ...
    Task<PagedEntities<T>> GetPaginatedAsync(int pageNumber, int itemsPerPage, CancellationToken cancellationToken = default);
    Task<PagedEntities<T>> GetPaginatedWhereAsync(Expression<Func<T, bool>> predicate, int pageNumber, int itemsPerPage, CancellationToken cancellationToken = default);
    Task<PagedEntities<TResult>> GetPaginatedMappedAsync<TResult>(int pageNumber, int itemsPerPage, Expression<Func<T, TResult>> select, CancellationToken cancellationToken = default);
    Task<PagedEntities<TResult>> GetPaginatedWhereMappedAsync<TResult>(Expression<Func<T, bool>> predicate, int pageNumber, int itemsPerPage, Expression<Func<T, TResult>> select, CancellationToken cancellationToken = default);
}